...
The tests under `tests/` use Node's built-in test runner with TypeScript type stripping. They cover all three curricula, source resolution, quiz behavior, sandbox guards, Python and TypeScript Worker protocols, result validation, and structural properties of every browser runtime.
...
- `lib/typescript-runner.ts`

The command explicitly includes only those four files and enforces 100% line, function, and branch coverage. This is **core-logic coverage**, not project-wide coverage.

Generated JavaScript sandbox code, `public/python-worker.mjs`, and `public/typescript-compiler-worker.js` are deliberately outside that threshold. Node structural checks verify required boundaries; Chromium verifies real runtime behavior, lazy loading, diagnostics, emit, execution, timeout, recovery, and every authored solution.
...
| `pnpm test:coverage` | 100% thresholds for the four explicit core files. |
...
| TypeScript compiler | Lazy 5.9.3 Worker, TS2322, no emit on error, recovery and Worker reuse | `typescript.spec.ts` | Yes | No | Automated |
